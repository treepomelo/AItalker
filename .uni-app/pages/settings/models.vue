    ...
    <view v-if="ready" class="grid">
      <view v-for="channel in channels" :key="channel.id" class="panel">
        ...
        <view v-if="channel.id==='speech'">
          ...
          <view class="help">统一输出 MP3。MiniMax 语速 0.5–2；OpenAI 兼容语速 0.25–4；腾讯云语速自动映射到 -2–6，以所选模型支持范围为准。</view>
        </view>
...
function protocolHelp(p){return {OPENAI_CHAT:'Bearer 鉴权；解析 choices.message / choices.delta。',ANTHROPIC:'x-api-key + anthropic-version；系统提示与 messages 分开。',GEMINI:'x-goog-api-key；转换 contents / parts，并独立生成流式地址。',OPENAI_SPEECH:'Bearer 鉴权；返回 MP3 二进制音频。',MINIMAX_SPEECH:'Bearer 鉴权；转换 voice_setting 并解码 hex 音频。',TENCENT_SPEECH:'TC3 签名鉴权；Key 填 SecretId:SecretKey，模型填数字音色 ID（如 601000），仅输出 mp3，长文本自动分段合成。'}[p]||''}
