...
public class KnowledgeBaseController {
    ...
    @GetMapping public Result list() { return Result.data(jdbc.queryForList("SELECT * FROM knowledge_base ORDER BY created_at")); }
    @PostMapping public Result create(@RequestBody Map<String,String> body) {
        String id=UUID.randomUUID().toString(); String name=body.getOrDefault("name","").trim();
    ...
    @PostMapping("/{id}/documents") public Result add(@PathVariable String id,@RequestBody Map<String,String> body) {
        requireBase(id); String content=body.getOrDefault("content","").trim();
    ...
    @GetMapping("/{id}/documents") public Result documents(@PathVariable String id) {
        requireBase(id); return Result.data(jdbc.queryForList("SELECT * FROM knowledge_document WHERE knowledge_base_id=?",id));
    }
